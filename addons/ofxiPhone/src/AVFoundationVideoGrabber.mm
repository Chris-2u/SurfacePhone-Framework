...
-(void) startCapture{
    ...
    [captureInput.device setAutomaticallyEnablesLowLightBoostWhenAvailable:false];
	...
	//if( [captureInput.device isFocusModeSupported:AVCaptureFocusModeContinuousAutoFocus] )
    [captureInput.device setFocusMode:AVCaptureFocusModeContinuousAutoFocus ];
}

-(void) lockExposureAndFocus:(BOOL)lock {
    CGPoint poi = CGPointMake(0.5, 0.5);
    
    [captureInput.device setFocusPointOfInterest:poi];
    [captureInput.device setExposurePointOfInterest:poi];

//    AVCaptureDeviceInput * _captureInput = (AVCaptureDeviceInput*) timer.userInfo;
    NSLog(@"Locked exposure and focus");
    if (lock) {
        ...
        // adjust to optimal focus and exposure
        [captureInput.device setExposureMode:AVCaptureExposureModeContinuousAutoExposure ];
        [captureInput.device setFocusMode:AVCaptureFocusModeContinuousAutoFocus ];
    }
}
