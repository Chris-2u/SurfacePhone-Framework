...
//    input.blurGaussian(7);

    if (_threshold != -1)
        input.threshold(_threshold);

//        input.adaptiveThreshold(31, 5, true, false);
//    if (HUEbased) {
//        input.dilate();
//        input.erode();
//        input.erode();
...
//    int i = 0; // use only first contour
    int j = 0; // and first finger as both are already sorted
        ofSetColor(221, 0, 204, 200);
        trackedBlobs[0].drawBox(0,0,width,height);
        ...
        if (trackedBlobs[i].nFingers > 0){
            ofDrawBitmapString("Hand with " + ofToString(trackedBlobs[i].nFingers) + " fingers", trackedBlobs[i].centroid.x * width, trackedBlobs[i].centroid.y * height + 15);
//            for (int j = 0; j < trackedBlobs[i].nFingers ; j++){
                ofFill();
                ofSetColor(i==0 && j==0 ? 0 : 255,i==0 && j==0 ? 255 : 0,0);
                ofCircle(trackedBlobs[i].fingers[j].x * width, trackedBlobs[i].fingers[j].y * height,20);
//            }
        }
    }
